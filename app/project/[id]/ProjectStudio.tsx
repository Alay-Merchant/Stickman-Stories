      ...
      <div className="mx-auto max-w-7xl px-5 py-7 sm:px-8">
        ...
        {tab === "Source" ? (
          <section className="rounded-3xl border-2 border-[#171717] bg-white p-6">
            ...
            <div className="mt-6 flex flex-wrap gap-4"><label className="flex items-center gap-2 rounded-xl border-2 border-[#171717] px-4 py-3 text-sm font-bold"><input checked={projectDraft.inputMode === "text"} onChange={() => setProjectDraft((current) => ({...current, inputMode: "text"}))} type="radio" /> Use source notes</label><label className="flex items-center gap-2 rounded-xl border-2 border-[#171717] px-4 py-3 text-sm font-bold"><input checked={projectDraft.inputMode === "reference_only"} onChange={() => setProjectDraft((current) => ({...current, inputMode: "reference_only"}))} type="radio" /> No source notes</label></div>
            {projectDraft.inputMode === "text" ? <Field label="Source notes"><textarea className="studio-input mt-2 min-h-80 font-mono text-sm" onChange={(event) => setSourceDraft(event.target.value)} value={sourceDraft} /></Field> : <p className="mt-6 rounded-xl bg-[#fff8d9] p-4 text-sm leading-6">Source notes are optional. Reference-only mode uses your brief as the starting point; verify every factual claim against public sources before publishing.</p>}
          </section>
        ) : null}
