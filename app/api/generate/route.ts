...
import {generateProject} from "@/lib/workflow";
...
export async function POST(request: Request) {
  try {
    ...
    return NextResponse.json(await generateProject(id));
  } catch (error) {
    return NextResponse.json({error: publicErrorMessage(error, "Generation failed. Check the local provider settings and try again.")}, {status: 400});
  }
}
